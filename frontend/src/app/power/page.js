...
export default function PowerPage() {
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        const data = await fetchProducts({ limit: 30, skip: 15 })
        setProducts(data.products || [])
      } catch (err) {
        console.error('Failed to load Power products:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="container mx-auto px-4 py-4">
      ...
      <div className='flex items-center justify-start gap-2'>
        ...
        <p className='text-sm text-green-700 font-medium'>Power</p>
      </div>

      {/* Logo */}
      <div className='container flex items-center justify-center overflow-hidden mt-2'>
        <Image className='h-12 lg:h-20 w-20 lg:w-50 py-1 px-1 border-2 border-gray-400 rounded-2xl bg-white shadow-2xs cursor-pointer' src={Power} alt='Power Logo' />
      </div>
      ...
      <div className="pt-8">
        <div className="flex items-center">
          ...
          <h2 className="text-lg lg:text-2xl font-bold mb-4 ml-2">Power</h2>
        </div>
