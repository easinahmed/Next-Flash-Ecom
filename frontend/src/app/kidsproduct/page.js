...
export default function KidsProductsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const data = await fetchProducts({ gender: 'kids', limit: 30 })
  ...
  return (
    <div className="container mx-auto px-4 py-4">
      <div className="flex items-center justify-start gap-2">
        ...
        <p className="text-sm text-green-700 font-medium">Kids Collection</p>
      </div>

      <div className="pt-8">
        <div className="flex items-center">
          <div className="h-[20px] w-[20px] bg-red-700 mb-4 rounded-3xl" />
          <h2 className="text-lg lg:text-2xl font-bold mb-4 ml-2">Kids Collection</h2>
        </div>
