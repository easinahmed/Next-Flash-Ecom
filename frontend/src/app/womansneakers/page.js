...
export default function WomanSneakersPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const data = await fetchProductsByCategory('womens-shoes', { limit: 30 })
        setProducts(data.products || [])
      } catch (err) {
        console.error('Failed to load woman sneakers:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="container mx-auto px-4 py-4">
      <div className="flex items-center justify-start gap-2">
        <Link href="/sneakerstudio"><p className="text-sm cursor-pointer hover:underline">Sneakerstudio</p></Link>
        <p className="text-sm">/</p>
        <p className="text-sm text-green-700 font-medium">Woman Sneakers</p>
      </div>

      <div className="pt-8">
        <div className="flex items-center">
          <div className="h-[20px] w-[20px] bg-red-700 mb-4 rounded-3xl" />
          <h2 className="text-lg lg:text-2xl font-bold mb-4 ml-2">Woman Sneakers</h2>
        </div>
